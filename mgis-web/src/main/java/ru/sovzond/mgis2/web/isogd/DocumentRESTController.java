...
public class DocumentRESTController implements Serializable {
    ...
    @RequestMapping(value = "/uploadDocumentContent")
    @Transactional
    public DocumentContent uploadCommonContent(@RequestBody MultipartFile file) {
        String contentType = file.getContentType();
        List<RepresentationFormat> list = representationFormatBean.find(contentType);
        if (list.size() == 1) {
